...
IF NOT DEFINED EMSCRIPTEN_SDK_DIR (
...
odin.exe build src\main_web -o:speed -target:js_wasm32 -build-mode:obj ^
                    -define:RAYLIB_WASM_LIB=env.o -define:RAYGUI_WASM_LIB=env.o ^
                    -vet -vet-packages:main ^
                    -collection:sol=extern/sol ^
                    -strict-style -out:%OUT_DIR%\game
IF %ERRORLEVEL% NEQ 0 exit /b 1

for /f %%i in ('odin.exe root') do set "ODIN_PATH=%%i"
...
set flags=-sUSE_GLFW=3 -sWASM_BIGINT -sWARN_ON_UNDEFINED_SYMBOLS=0 -sASSERTIONS --shell-file src\main_web\index_template.html --preload-file assets -sALLOW_MEMORY_GROWTH=1 -sINITIAL_HEAP=16777216 -sSTACK_SIZE=65536
...
cmd /c emcc -O3 -o %OUT_DIR%\index.html %files% %flags%
